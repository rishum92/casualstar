...
@section('content')
  ...
  <div data-ng-controller="DashboardController">
    <section class="browse" data-ng-init="filtersSet={{$filters}}">
	...
	<div class="bar">
      ...
      <div class="wrap"> 
        <h1>{{ Lang::get('messages.browseMembers') }}</h1>
        ...
        <div class="options-container">
          <div class="options">  
           ...
           <!--      <div class="form-group check-group">
                  ...
                  <label for="gender_filter1" ><span>{{ Lang::get('messages.male') }}</span></label><br>
                </div>
                <div class="form-group check-group">
                  <input name="gender_filter2" data-ng-change="filterUsers()" data-ng-model="genderFemale" id="gender_filter2" type="checkbox">
                  <label for="gender_filter2" ><span>{{ Lang::get('messages.female') }}</span></label> 
                </div> --> 
                <div class="form-group images-only"> 
                  ...
                  <label for="image_filter"><span></span>{{ Lang::get('messages.withImagesOnly') }}</label>
                </div> 
                <div class="form-group check-group online-now">
                  ...
                  <label for="online_filter"><span></span>{{ Lang::get('messages.onlineNow') }}</label>
                </div>
                <div class="form-group age-group range-group first-age-group block-flex wrap-flex vertical-center-flex">
                  <label>{{ Lang::get('messages.minAge') }}</label>
                ...
                <div class="form-group age-group range-group block-flex wrap-flex vertical-center-flex">
                  <label>{{ Lang::get('messages.maxAge') }}</label>
              ...
              <div class="block-flex wrap-flex vertical-center-flex range-row_custom"> 
                <div class="form-group _custom_one range-group block-flex wrap-flex vertical-center-flex" >
                  <label>{{ Lang::get('messages.lookingFor') }}</label>
                ...
                <div class="form-group _custom_two range-group range-group block-flex wrap-flex vertical-center-flex" ng-show="getToggle()">
                  <label>{{ Lang::get('messages.distance') }}</label>
              ...
              <div data-ng-if="totalBrowse > 0" class="member">
                <div class="image">
                  ...
                  <a href="/users/[[user.username]]">
                    <img data-ng-if="user.img != ''" data-ng-src="[[getUserPhotoPreviewUrl(user)]]" alt="[[user.username]]" />
                    <img data-ng-if="user.img ==''" data-ng-src="img/male.jpg" alt="[[user.username]]" />
                  </a>
                </div><!--
              }
                ...
                <p>[[user.age]] {{ Lang::get('messages.yearsOld') }}</p>
    ...
    <section class="rec-members fav-members">
      <div class="wrap">
        <h3 data-ng-if="favorites.length > 0">{{ Lang::get('messages.yourFavoriteUsers') }}</h3>
        <div data-ng-if="favorites.length > 0" class="row">
          <div class="col-lg-3 col-md-3 col-xs-4"  data-ng-repeat="user in favorites">
            <div class="member">
              ...
              <a href="/users/[[user.username]]" class="profile-link">{{ Lang::get('messages.seeProfile') }} <i class="ion-arrow-right-c"></i></a>
              <button type="button" class="edit-button" mwl-confirm="" title="{{ Lang::get('messages.removeFavorite') }}" message="" confirm-text="<i class='ion-android-done'></i>" cancel-text="<i class='ion-android-close'></i>" placement="top" on-confirm="destroyFavorite(user)" on-cancel="vm.cancelClicked = true" confirm-button-type="danger" cancel-button-type="default" ng-click="vm.confirmClicked = false; vm.cancelClicked = false"><i class="ion-trash-a"></i></button>
            </div>
          </div>
        </div>
        <h3 data-ng-if="favorites.length == 0">{{ Lang::get('messages.noFavoriteUsers') }}</h3>
